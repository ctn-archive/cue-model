...
class SimWeightSymmetryLearning(nengo.builder.Operator):
    ...
    def make_step(self, signals, dt, rng):
        ...
        def step_assoc_learning():
            ...
            decoders[...] += 2. * alpha * scale * np.outer(pre, pre)
...
def build_weight_symmetry_learning(model, weight_symmetry_learning, rule):
    ...
    pre_filt = model.build(nengo.Lowpass(0.01), pre)
    ...
    model.add_op(SimWeightSymmetryLearning(
        weight_symmetry_learning.learning_rate, base_decoders, pre_filt,
        decoders, scale))
...
with nengo.Network(seed=1) as model:
    ...
    nengo.Connection(square.add_output('square', np.square), conn.learning_rule, transform=np.ones((1, d)))

    with nengo.presets.ThresholdingEnsembles(0.):
        prod = nengo.networks.Product(400, n * d)
    nengo.Connection(post, prod.input_a, synapse=None, transform=3e4)
    nengo.Connection(pre.neurons, prod.input_b, transform=0.001)
    nengo.Connection(prod.output, conn.learning_rule, transform=-1e-1*np.ones((1, n * d)) / 3.)
    ...
    p_act = nengo.Probe(pre.neurons, sample_every=sample_every)
    p_prod = nengo.Probe(prod.output, sample_every=sample_every)
    p_a = nengo.Probe(prod.input_a, sample_every=sample_every)
    p_b = nengo.Probe(prod.input_b, sample_every=sample_every)
...
np.savez(
    'weightsym_neural.npz',
    t=sim.trange(), W=W, def_conn_weights=sim.data[def_conn].weights,
    decoders=sim.data[p_decoders], post=sim.data[p_post],
    def_post=sim.data[p_def_post], act=sim.data[p_act],
    prod=sim.data[p_prod],
    err=np.linalg.norm(W[None, :, :] - sim.data[p_decoders], axis=(1, 2)),
    a=sim.data[p_a], b=sim.data[p_b])
